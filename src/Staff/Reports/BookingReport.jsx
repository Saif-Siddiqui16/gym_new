...
const BookingReport = () => {
    ...
    const bookingStats = [
        { label: 'Total Bookings', value: bookings.length.toString(), icon: ClipboardList, bg: 'bg-primary-light', color: 'text-primary' },
        ...
        <div className="p-0 md:p-8 bg-gray-50 min-h-screen font-sans staffdashboard-reports">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="h-10 px-4 bg-primary text-white rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-primary shadow-md transition-all">
                        <FileText size={16} />
                        Export PDF
                    </button>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100" style={{ overflow: 'visible', zIndex: 10, position: 'relative' }}>
                {/* Filters Section */}
                <div className="p-4 sm:p-6 border-b border-gray-50 flex flex-col lg:flex-row gap-4 items-center">
                            ...
                            {loading ? (
                                ...
                                .map((b) => (
                                    <tr key={b.id} className="hover:bg-gray-50 transition-all duration-200 group">
                                        <td data-label="Booking ID" className="text-xs font-bold text-primary">{b.id}</td>
                                        ...
                                        <td data-label="Status" className="text-center">
                                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${b.status === 'Completed' ? 'bg-green-100 text-green-700' :
                                                b.status === 'Cancelled' ? 'bg-red-100 text-red-700' :
                                                    'bg-violet-100 text-primary-hover'
                                                }`}>
                                                {b.status}
                                            </span>
                                        </td>
