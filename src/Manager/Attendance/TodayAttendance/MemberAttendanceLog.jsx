...
import { Calendar, Search, Filter, Download, Clock, MapPin, User, Activity, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react';
...
const MemberAttendanceLog = () => {
    ...
    useEffect(() => {
        loadData();
    }, [searchTerm, filterStatus, selectedDate, currentPage]);
    ...
    const handleExport = () => {
        exportPDF(members, 'MemberAttendanceLog');
    };
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            ...
            case 'checked-out':
            case 'Checked Out':
                return <span className="inline-flex items-center px-3 py-1.5 rounded-xl bg-gradient-to-r from-primary to-primary text-white text-xs font-black shadow-lg shadow-primary/30/50 hover:scale-110 hover:shadow-xl transition-all duration-300 cursor-pointer">Checked Out</span>;
            default:
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-primary-light/30 p-4 sm:p-6">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-slate-600 text-xs sm:text-sm mt-1">Monitor member visits and class attendance</p>
                            </div>
                        </div>
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            <button
                                onClick={handleExport}
                                className="group flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-primary to-primary text-white rounded-xl text-sm font-bold shadow-xl shadow-primary/30/50 hover:shadow-2xl hover:shadow-primary/30/60 hover:scale-105 transition-all"
                            >
                            ...
                            <button
                                ...
                                className={`group flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-bold shadow-lg hover:shadow-xl hover:scale-105 transition-all ${showFilters
                                    ? 'bg-gradient-to-r from-primary to-primary text-white shadow-primary/30/50'
            ...
            {showFilters && (
                <div className="group relative bg-white rounded-2xl shadow-xl border border-slate-100 p-4 sm:p-6 mb-6 overflow-visible hover:shadow-2xl transition-all duration-300" style={{ zIndex: 50 }}>
                    <div className="absolute inset-0 bg-gradient-to-br from-primary-light/30 to-purple-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl"></div>
                    <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                        <div className="relative group/input">
                            <Search size={18} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 group-hover/input:text-primary transition-colors duration-300" strokeWidth={2.5} />
                            <input
                                type="text"
                                placeholder="Search member name..."
                                className="w-full pl-11 pr-4 py-3 border-2 border-slate-200 rounded-xl focus:border-primary focus:ring-2 focus:ring-violet-200 hover:border-violet-300 transition-all duration-300 font-semibold text-sm"
                                value={searchTerm}
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full md:col-span-2">
                            ...
                            <div className="relative group/input w-full sm:w-auto">
                                <input
                                    type="date"
                                    className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-primary focus:ring-2 focus:ring-violet-200 hover:border-violet-300 transition-all duration-300 font-semibold text-sm"
                                    value={selectedDate}
                                    onChange={(e) => { setSelectedDate(e.target.value); setCurrentPage(1); }}
                                />
                                <Calendar size={16} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 pointer-events-none group-hover/input:text-primary transition-colors duration-300" strokeWidth={2.5} />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="hidden md:block overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50/50 border-b border-slate-100">
                                <tr>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-primary transition-colors duration-300 cursor-pointer">Member Name</th>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-primary transition-colors duration-300 cursor-pointer">Membership ID</th>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-primary transition-colors duration-300 cursor-pointer">Plan</th>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-primary transition-colors duration-300 cursor-pointer">Check-In</th>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-primary transition-colors duration-300 cursor-pointer">Check-Out</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {loading ? (
                                    <tr>
                                        <td colSpan="6" className="px-6 py-12 text-center text-sm text-slate-400">
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <span className="font-semibold">Loading member attendance...</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : members.length > 0 ? (
                                    members.map((record) => (
                                        <tr key={record.id} className="group/row hover:bg-gradient-to-r hover:from-primary-light/50 hover:to-purple-50/50 transition-all duration-300 cursor-pointer hover:shadow-md">
                                            ...
                                            <td className="px-6 py-4 text-slate-600 text-sm font-mono font-semibold group-hover/row:text-primary transition-colors duration-300">
                                                {record.membershipId || String(record.id || '').substring(0, 8).toUpperCase()}
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className="inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-700 group-hover/row:bg-gradient-to-r group-hover/row:from-violet-100 group-hover/row:to-purple-100 group-hover/row:text-primary-hover group-hover/row:scale-110 transition-all duration-300">
                                                    {record.plan || 'Standard'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-slate-700 text-sm font-semibold group-hover/row:text-primary transition-colors duration-300">
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-6 py-12 text-center text-sm text-slate-400">
                                            No member attendance logs found for this date.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                    ...
                    <div className="md:hidden p-4 space-y-4">
                        {loading ? (
                            <div className="py-12 text-center text-sm text-slate-400">
                                <div className="flex flex-col items-center gap-3">
                                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                                    <span className="font-semibold text-primary">Loading data...</span>
                                </div>
                            </div>
                        ) : members.length > 0 ? (
                            members.map((record) => (
                                <MobileCard
                                    key={record.id}
                                    title={record.name}
                                    subtitle={record.membershipId || String(record.id || '').substring(0, 8).toUpperCase()}
                                    badge={record.status}
                                    ...
                                    fields={[
                                        { label: 'Plan', value: record.plan || 'Standard' },
...
export default MemberAttendanceLog;
