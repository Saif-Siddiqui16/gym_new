            ...
            <div className="bg-white rounded-lg shadow-md p-4 mb-6">
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1 relative group">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 transition-all duration-300 group-focus-within:scale-110 group-focus-within:text-primary" />
                        <input
                            type="text"
                            placeholder="Search by invoice number or gym name..."
                            className="superadmindashboard-invoices-search w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-300 focus:scale-[1.02] focus:shadow-lg hover:border-violet-300"
                            value={searchTerm}
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {paginatedInvoices.length > 0 ? (
                                paginatedInvoices.map((invoice) => (
                                    <tr key={invoice.invoiceNo} className="hover:bg-primary-light/30 hover:shadow-md hover:scale-[1.005] transition-all duration-300 cursor-pointer group">
                                        <td data-label="Invoice No">
                                            <span className="text-sm font-semibold text-primary transition-all duration-300 group-hover:scale-105 group-hover:text-primary-hover">
                                                {invoice.invoiceNo}
                                            </span>
                                        </td>
                                        <td data-label="Gym Name">
                                            <span className="text-sm text-gray-800 font-medium transition-colors duration-300 group-hover:text-primary">
                                                {invoice.gymName}
                                            </span>
                                        </td>
                                        <td data-label="Amount">
                                            <span className="text-sm font-semibold text-gray-900 transition-colors duration-300 group-hover:text-primary">
                                                {invoice.amount}
                                            </span>
                                        </td>
                                        ...
                                        <td data-label="Actions" className="text-right">
                                            <div className="flex gap-2 justify-end">
                                                <button
                                                    className="bg-primary hover:bg-primary text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-300 flex items-center gap-1 hover:scale-110 hover:shadow-lg hover:-translate-y-0.5 group/btn"
                        ...
                        <div className="flex gap-2 order-1 md:order-2 overflow-x-auto max-w-full pb-1">
                            ...
                            <div className="flex gap-2">
                                {[...Array(totalPages)].map((_, index) => (
                                    <button
                                        key={index + 1}
                                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 hover:scale-110 ${currentPage === index + 1 ? 'bg-primary text-white shadow-md hover:shadow-lg' : 'border border-gray-300 text-gray-700 hover:bg-gray-100 hover:shadow-sm'}`}
                    ...
                    <div className="flex items-center justify-center min-h-screen ">
                        ...
                        <div className="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all duration-500 hover:scale-[1.02] sm:my-8 sm:align-middle sm:max-w-lg sm:w-full animate-slide-up">
                            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="sm:flex sm:items-start text-primary border-b border-gray-100 pb-4 mb-4">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                                        <div className="p-3 rounded-lg hover:bg-primary-light/50 transition-all duration-300">
                            ...
                            <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-2xl">
                                <button
                                    ...
                                    className="w-full inline-flex justify-center rounded-lg bg-primary hover:bg-primary-hover text-white text-base font-medium shadow-sm px-4 py-2 sm:ml-3 sm:w-auto sm:text-sm transition-all duration-300 flex items-center gap-2 hover:scale-105 hover:shadow-lg hover:-translate-y-0.5 group/btn"
                                >
