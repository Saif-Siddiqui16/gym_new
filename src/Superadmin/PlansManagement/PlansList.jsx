...
const PlansList = () => {
    ...
    const LimitBadge = ({ limit, icon: Icon }) => (
        <div className="flex items-center gap-1.5 px-2 py-1 bg-slate-50 border border-slate-100 rounded-lg text-slate-600">
            ...
            <span className="text-[10px] font-black uppercase">
                {limit?.isUnlimited ? <Infinity size={14} className="text-violet-600" /> : (limit?.value || '0')}
            </span>
        </div>
    );
    ...
    return (
        <div className="bg-gradient-to-br from-gray-50 via-white to-violet-50/30 h-full w-full">
            {/* Premium Header */}
            <div className="mb-8 relative">
                ...
                <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-100 p-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <button
                            className="group relative px-6 py-3 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl hover:shadow-purple-500/50 hover:scale-105 hover:-translate-y-1 transition-all duration-300 flex items-center gap-2"
            ...
            <div className="mb-6 bg-white/60 backdrop-blur-md rounded-xl shadow-lg border border-white/50 p-5 relative overflow-visible">
                <div className="flex flex-col sm:flex-row gap-4 overflow-visible">
                    <div className="relative flex-1 group">
                        <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-violet-500 transition-all duration-300" />
                        <input
                            type="text"
                            placeholder="Search plans..."
                            className="w-full pl-12 pr-4 py-3 bg-white/80 border-2 border-gray-200 rounded-xl text-sm focus:outline-none focus:border-violet-500 transition-all duration-300"
                            value={searchTerm}
                                ...
                                <tr key={plan.id} className="group hover:bg-slate-50/50 transition-all duration-200">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center font-black text-lg shadow-inner">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                            <button onClick={() => handleEdit(plan.id)} className="p-2.5 text-slate-400 hover:text-violet-600 hover:bg-violet-50 hover:scale-110 rounded-xl transition-all shadow-sm bg-white border border-slate-100">
                                                <Edit size={16} />
                                            </button>
