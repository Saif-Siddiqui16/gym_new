...
const TrainerAssignment = () => {
    ...
    return (
        <div className="fade-in px-4 sm:px-6 lg:px-8 py-6" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <Card className="mb-8 border-violet-100 bg-violet-50/30">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 'var(--space-4)' }}>
                {trainers.map(trainer => {
                    ...
                    return (
                        ...
                        >
                            {isSelected && (
                                <div className="absolute top-4 right-4 text-violet-600 animate-in zoom-in duration-300">
                                    <CheckCircle size={24} />
                                </div>
                            )}
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-colors duration-300 ${isSelected ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-500 group-hover:bg-violet-100 group-hover:text-violet-600'}`}>
                                <User size={24} />
                            </div>
                            <div className="font-bold text-gray-900 group-hover:text-violet-600 transition-colors duration-300">{trainer.name}</div>
