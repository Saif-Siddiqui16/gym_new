                    ...
                    <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full md:col-span-2">
                            ...
                            <div className="relative group/input w-full sm:w-auto">
                                <input
                                    type="date"
                                    className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-violet-500 focus:ring-2 focus:ring-violet-200 hover:border-violet-300 transition-all duration-300 font-semibold text-sm"
                                    value={selectedDate}
                                    onChange={(e) => { setSelectedDate(e.target.value); setCurrentPage(1); }}
                                />
                                <Calendar size={16} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 pointer-events-none group-hover/input:text-violet-600 transition-colors duration-300" strokeWidth={2.5} />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="hidden md:block overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50/50 border-b border-slate-100">
                                <tr>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-violet-600 transition-colors duration-300 cursor-pointer">Member Name</th>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-violet-600 transition-colors duration-300 cursor-pointer">Membership ID</th>
                                    <th className="px-6 py-4 font-black text-slate-400 uppercase tracking-wider text-xs hover:text-violet-600 transition-colors duration-300 cursor-pointer">Plan</th>
                                ...
                                {loading ? (
                                    ...
                                    members.map((record) => (
                                        <tr key={record.id} className="group/row hover:bg-gradient-to-r hover:from-violet-50/50 hover:to-purple-50/50 transition-all duration-300 cursor-pointer hover:shadow-md">
                                            ...
                                            <td className="px-6 py-4 text-slate-600 text-sm font-mono font-semibold group-hover/row:text-violet-600 transition-colors duration-300">
                                                {record.membershipId || String(record.id || '').substring(0, 8).toUpperCase()}
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className="inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-700 group-hover/row:bg-gradient-to-r group-hover/row:from-violet-100 group-hover/row:to-purple-100 group-hover/row:text-violet-700 group-hover/row:scale-110 transition-all duration-300">
